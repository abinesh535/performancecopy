...
exports.recordTimeout = recordTimeout;
...
function recordTime(ms, userId) {
    append({ type: 'time', value: ms, userId });
}
function recordError(userId) {
...
function getData() {
    ...
    const lines = fs_1.default.readFileSync(FILE, 'utf-8')
        .trim()
        .split('\n')
        .filter(Boolean);
    const data = {
...
async function calculate(data) {
    ...
    const getPercentile = (p) => sorted.length
